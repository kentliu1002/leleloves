...
export function deletePolicy(row) {
  return row?.source === 'government' ? 'disable' : 'delete'
}
...
export async function fetchGovernmentYear(year, request = fetch) {
  ...
  if (payload?.year === year && payload.papers?.length === 0 && payload.days?.length === 0) return null
  return parseGovernmentCalendar(payload)
}

export function syncYears(date = new Date()) {
  const year = new Date(date.getTime() + 8 * 60 * 60 * 1000).getUTCFullYear()
  return [year, year + 1]
}

export async function syncGovernmentCalendar({ years, fetchYear = fetchGovernmentYear, repository }) {
  try {
    const calendars = (await Promise.all(years.map(year => fetchYear(year)))).filter(Boolean)
