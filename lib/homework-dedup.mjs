...
export function submissionId({ filename, content, date, fileHashes = [] }) {
  const identity = fileHashes.length
    ? `files:${JSON.stringify([...fileHashes].sort())}|text:${(content || '').trim()}`
    : (filename || '').trim() ? `file:${filename.trim()}` : `text:${(content || '').trim()}`
