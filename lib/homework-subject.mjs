  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await request('https://ark.cn-beijing.volces.com/api/coding/v3/chat/completions', {
        ...
        body: JSON.stringify({ model: 'doubao-seed-evolving', messages: [{ role: 'user', content }], thinking: { type: 'disabled' }, max_tokens: 32 })
      })
      ...
      console.error(`[subject] attempt ${attempt + 1} failed:`, error.message, { cause: error.cause?.code || error.cause?.name, details: error.cause?.errors?.map(item => item.code) })
    }
  }
  if (fallback !== '其它') return fallback
  throw new Error('识别服务暂时不可用，请稍后重试')
